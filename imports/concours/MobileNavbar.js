...
class MobileNavbar extends Component{
  ...
  createDropdownItems(items, level){
    return(items.map((subitem, i)=>{
      ...
      return(
        <LinkContainer style={indentStyle(level)} onClick={this.toggleMenuOpen} to={'/concours/folder/'+subitem.id} key={'menuItem'+subitem.label+'-'+i}>
      ...
      <nav id='mobileNavbar' className='navbar navbar-default'>
        <div className='container-fluid'>
          <div className='navbar-header'>
            <Link style={{fontSize: '1.3em'}} className='navbar-brand' to='/concours'>Concours Media Center</Link>
